        ...
        <Box :title="type">
            <template #content>
                <div class="d-flex gap-2 flex-wrap">
                    <div class="btn-group" v-for="(subItem, subItemIndex) in $store.state.subItems.data[type]">
                        <router-link type="button"
                            :class="'px-1 py-0 border rounded btn icon ' + ($route.params.id === item._id ? 'fw-bold ' : '') + ($route.params.subitemid === subItem._id ? 'selected ' : '')"
                            :to="'/' + $route.path.split('/')[1] + '/' + item._id + '/' + type + '/' + subItem._id">{{
...
import AddNewSubItem from './AddNewSubItem.vue';
export default {
    props: ['item', "type"],
    components: {
        AddNewSubItem
